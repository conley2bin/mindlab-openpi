...
- once `MINT_OPENPI_REMOTE_SMOKE=1`, missing `MINT_OPENPI_REMOTE_BASE_URL` or malformed / non-finite `MINT_OPENPI_REMOTE_TIMEOUT_S` are treated as `environment` failures instead of silent skips
