...
| Mint OpenPI service plane | `src/mint` | config + schema + inference bridge exist | gated OpenPI route family, schema family, runtime bridge | `src/mint/tests/test_openpi_app_registration.py`, `src/mint/tests/test_openpi_config_validation.py`, `src/mint/tests/test_openpi_service_contract.py`, `src/mint/tests/test_openpi_does_not_pollute_tinker_types.py`, `src/mint/tests/test_openpi_runtime_bridge.py`, `src/mint/tests/test_issue_281_scheduler_and_healthz.py` | none | current Mint Tinker-compatible anchors above | OpenPI disabled 仍是默认行为；artifact proxy 和 training endpoint 还没开始；`src/mint/tinker_server/models/types.py` 仍保持 token-only。 |
...
| --- | --- | --- | --- |
| deterministic inference-only closed loop | `src/openpi/src/openpi/integration/runtime_test.py`, `src/openpi/src/openpi/integration/artifacts_test.py`, `src/mint/tests/test_openpi_service_contract.py`, `src/mint/tests/test_openpi_runtime_bridge.py` | yes | in progress |
