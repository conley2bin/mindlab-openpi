...
| Cross-repo deterministic closed loop | shared | fake-runtime status + inference loop exists | Mint status probe plus fake runtime to Mint service to Toolkit SDK inference path | `src/mint/tests/test_openpi_cross_repo_closed_loop.py` | none | Mint and Toolkit existing anchors | harness 当前落在 `src/mint/tests/`；使用 fake runtime，不依赖外网和真实 checkpoint，并验证了 status contract、structured payload 与 `reset()` lifecycle。 |
